document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!messageBox) {
    console.warn('deleteMessage div no encontrado. Agrega <div id="deleteMessage" class="delete-message"></div> antes de tus scripts.');
  }

  // El target efectivo (si existe el <a> dentro) o el div
  const target = deleteLink || deleteBtnDiv;
  if (!target) {
    console.warn('No se encontró .btn-delete ni .btn-delete a. Revisa el HTML.');
  } else {
  ...
  if (!cancelTarget) {
    console.warn('No se encontró .btn-cancell-delete ni .btn-cancell-delete a. Revisa el HTML.');
  } else {
